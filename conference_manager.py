...
class ConferenceManager(object):
    ...
    def create_sessions_fill_sessions(self):
        session_list = []
        ...
        self.validate_network_start_time(networking_start_time)
    ...
    try:
        ...
        if len(task_list) > 0:
            ...
            while any(task.is_scheduled == 0 for task in task_list):
                session_list = app.create_sessions_fill_sessions()
                app.create_track_and_fill_track(conference, session_list)
            app.print_sessions(conference)
        else:
